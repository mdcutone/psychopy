...
def xyz2luv(xyz, whitePoint=ILLUMINANT_D65, exact=True):
    """Convert CIE-XYZ color coordinates to CIE-L*u*v* (1976) color space.
    ...
    -------
    ndarray
        Array of CIE L*u*v* (1976) colors coordinates with similar shape to
        `xyz`.

    See Also
    --------
    luv2xyz : Inverse transformation of this function.
    ...
    demom = xyz[:, 0] + 15 * xyz[:, 1] + 3 * xyz[:, 2]
    up = (4 * xyz[:, 0]) / demom
    vp = (9 * xyz[:, 1]) / demom
    demom = whitePoint[0] + 15 * whitePoint[1] + 3 * whitePoint[2]
    up_ref = (4 * whitePoint[0]) / demom
    vp_ref = (9 * whitePoint[1]) / demom

    # compute LUV
    luv = numpy.zeros_like(xyz)
    luv[:, 0] = numpy.where(yr > eta, 116 * numpy.cbrt(yr) - 16, kappa * yr)
    luv[:, 1] = 13 * luv[:, 0] * (up - up_ref)
    luv[:, 2] = 13 * luv[:, 0] * (vp - vp_ref)

    # make the output match the dimensions/shape of input
    if orig_dim == 1:
        luv = luv[0]
    elif orig_dim == 3:
        luv = numpy.reshape(luv, orig_shape)
...
def blackbody(temp, method='kang'):
    ...
    temp : array_like or float
        ...
        ndarray.
    method : str
        Approximation method to use. Options are 'kang'[1]_ and 'ptb' which uses
        the same method as PsychToolbox's `GenerateCIEDay` function. The range
    ...
    ndarray
        ...
        the same index. If any temperatures fall outside of the range of a given
        method, the computed coordinate will be (0, 0).
    ...
    You can also specify an array of temperatures like this::
        ...
        #  [0.36071018 0.36349393]
        #  [0.38045969 0.37668534]]
    ...
    """
    # make sure we have an array with a `float` type
    temp = numpy.asarray(
    ...
    if method == 'kang':
        ...
        # upper range of temperatures between 4000K and 25000K
        temp_upper = numpy.where(numpy.logical_and(temp >= 4000., temp <= 25000.))
        if temp_upper:
            # compute x
            coefs = numpy.array((-3.0258469, 2.1070379, 0.2226347, 0.24039))
            to_return[temp_upper, 0] = numpy.sum(
                temp_t[temp_upper] * coefs, axis=1)

            # compute y
            xd2 = to_return[temp_upper, 0]
            coefs = numpy.array((3.0817580, -5.8733867, 3.75112997, -0.37001483))
            to_return[temp_upper, 1] = \
                xd2 ** 3 * coefs[0] + \
                xd2 ** 2 * coefs[1] + \
                xd2 * coefs[2] + coefs[3]

            # exit if there are no other ranges, don't bother testing for them
            if len(temp_upper[0]) == temp.size:
                return numpy.squeeze(to_return)

        # lower range of temperatures between 1677K and 4000K
        temp_lower = numpy.where(numpy.logical_and(temp >= 1677., temp < 4000.))
        if temp_lower:
            # compute x
            to_return[temp_lower, 0] = numpy.sum(
                temp_t[temp_lower] * numpy.array(
                    (-3.0258469, 2.1070379, 0.2226347, 0.24039)), axis=1)

            # compute y for sub ranges
            sub_high = numpy.where(numpy.logical_and(temp >= 2222., temp < 4000.))
            if sub_high:
                xd2 = to_return[sub_high, 0]
                coefs = numpy.array(
                    (-0.9549476, -1.37418593, 2.09137015, -0.16748867))
                to_return[sub_high, 1] = \
                    xd2 ** 3 * coefs[0] + \
                    xd2 ** 2 * coefs[1] + \
                    xd2 * coefs[2] + coefs[3]

            sub_low = numpy.where(numpy.logical_and(temp >= 1677., temp < 2222.))
            if sub_low:
                xd2 = to_return[sub_low, 0]
                coefs = numpy.array(
        ...
        temp_upper = numpy.where(numpy.logical_and(temp >= 7000., temp <= 25000.))
        temp_lower = numpy.where(numpy.logical_and(temp >= 4000., temp < 7000.))
        ...
        if temp_lower:
            ...
            to_return[temp_lower, 0] = numpy.sum(
                temp_t[temp_lower] * coefs, axis=1)

        xd = to_return[:, 0]
        to_return[:, 1] = -3. * (xd ** 2) + 2.870 * xd - 0.275
