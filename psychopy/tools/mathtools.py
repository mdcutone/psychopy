...
def applyQuat(q, points, out=None, dtype=None):
    ...
    This is similar to using `applyMatrix` with a rotation modelMatrix. However, it
    ...
    Show that you get the same result as a rotation modelMatrix::

        axis = [0., 0., -1.]
        angle = -90.0
        rotMat = rotationMatrix(axis, angle)[:3, :3]  # rotation sub-modelMatrix only
...
def quatToMatrix(q, out=None, dtype=None):
    """Create a rotation modelMatrix from a quaternion.
    ...
    -------
    ndarray or None
        4x4 rotation modelMatrix in row-major order.

    Examples
    --------
    Convert a quaternion to a rotation modelMatrix::

        point = [0., 1., 0., 1.]  # 4-vector form [x, y, z, 1.0]
        ori = [0., 0., 0., 1.]
        rotMat = matrixFromQuat(ori)
        # rotate 'point' using modelMatrix multiplication
...
def scaleMatrix(s, out=None, dtype=None):
    """Create a scaling modelMatrix.

    The resulting modelMatrix is the same as a generated by a `glScale` call.
    ...
    s : array_like, float or int
        ...
        anisotropic scaling modelMatrix if any of the values differ.
    out : ndarray, optional
        Optional output array. Must be same `shape` and `dtype` as the expected
        output if `out` was not specified.
    dtype : dtype or str, optional
    ...
    -------
    ndarray
        4x4 scaling modelMatrix in row-major order.
...
def rotationMatrix(angle, axis=(0., 0., -1.), out=None, dtype=None):
    """Create a rotation modelMatrix.

    The resulting modelMatrix will rotate points about `axis` by `angle`. The
    resulting modelMatrix is similar to that produced by a `glRotate` call.
    ...
    -------
    ndarray
        4x4 scaling modelMatrix in row-major order. Will be the same array as `out`
        if specified, if not, a new array will be allocated.

    Notes
    -----
    * Vector `axis` is normalized before creating the modelMatrix.
...
def translationMatrix(t, out=None, dtype=None):
    """Create a translation modelMatrix.

    The resulting modelMatrix is the same as generated by a `glTranslate` call.
    ...
    -------
    ndarray
        4x4 translation modelMatrix in row-major order. Will be the same array as
...
def invertMatrix(m, homogeneous=False, out=None, dtype=None):
    """Invert a 4x4 modelMatrix.

    Parameters
    ----------
    m : array_like
        4x4 modelMatrix to invert.
    homogeneous : bool, optional
        Set as ``True`` if the input modelMatrix specifies affine (homogeneous)
    ...
    -------
    ndarray
        4x4 modelMatrix which is the inverse of `m`
...
def concatenate(matrices, out=None, dtype=None):
    """Concatenate modelMatrix transformations.

    Combine 4x4 transformation matrices into a single modelMatrix. This is similar to
    what occurs when building a modelMatrix stack in OpenGL using `glRotate`,
    ...
    -------
    ndarray
        Concatenation of input matrices as a 4x4 modelMatrix in row-major order.

    Examples
    --------
    Create an SRT (scale, rotate, and translate) modelMatrix to convert model-space
    ...
    Create a model-view modelMatrix from a world-space pose represented by an
    orientation (quaternion) and position (vector). The resulting modelMatrix will
    transform model-space coordinates to eye-space::
        ...
        # create model modelMatrix
        R = matrixFromQuat(stimOri)
        T = translationMatrix(stimPos)
        M = concatenate(R, T)  # model modelMatrix

        # create a view modelMatrix, can also be represented as 'pos' and 'ori'
        eyePos = [0., 1.5, 0.]
        eyeFwd = [0., 0., -1.]
        eyeUp = [0., 1., 0.]
    ...
    You can put the created modelMatrix in the OpenGL modelMatrix stack as shown below.
    Note that the modelMatrix must have a 32-bit floating-point data type and needs
    ...
    concatenating the projection, view, and model matrices::
        ...
        # compute projection modelMatrix, functions here are from 'viewtools'
        screenWidth = 0.52
        screenAspect = w / h
        scrDistance = 0.55
...
def applyMatrix(m, points, out=None, dtype=None):
    """Apply a transformation modelMatrix over a 2D array of points.
    ...
    Examples
    --------
    Transform an array of points by some transformation modelMatrix::
        ...
        M = concatenate([S, R, T])  # create transform modelMatrix
    ...
    Extract the 3x3 rotation sub-modelMatrix from a 4x4 modelMatrix and apply it to
    points. Here the result is written to a pre-allocated array::
        ...
        M3x3 = M[:3, :3]  # extract rotation groups from the 4x4 modelMatrix
...
def posOriToMatrix(pos, ori, out=None, dtype=None):
    """Convert a rigid body pose to a 4x4 transformation modelMatrix.
    ...
    -------
    ndarray
        4x4 transformation modelMatrix.
...
def transform(pos, ori, points, out=None, dtype=None):
    ...
    You can get the same results as the previous example using a modelMatrix by doing
    the following::
    ...
    can skip the costly modelMatrix creation process by using `transform`.
